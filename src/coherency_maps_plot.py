...
def global_plots_with95ci(output_dirs, datasets, bands, seasons,
    ...
    if plot_raw_lags:
        ...
        norm = mpl.colors.Normalize(vmin=0.8, vmax=1.0, clip=False)
    ...
    add_date(fig)
    fig.suptitle(f"Mean coherency {response_var} relative to {reference_var}")

    axgr = AxesGrid(fig, 111,
                    axes_class=axes_class,
                    nrows_ncols=(len(bands), len(seasons)),
                    axes_pad=0.5,
                    label_mode="keep",
                    **kw_axesgrid)
    ...
    for ax, cax, (season, (band_lower, band_upper)) in zip(axgr, axgr.cbar_axes, product(seasons, bands)):
        ...
        file_coherency = os.path.join(lag_data_dir, f'coherency_{season}_{band_lower}-{band_upper}.npy')
        ...
        coherency = np.load(file_coherency)
        ...
        if plot_raw_lags:
            ...
            field_plot = np.ones_like(lag) * np.nan
            field_plot[positive_confidence_interval] = 2
            field_plot[negative_confidence_interval] = 0
            field_plot[confidence_interval_overlaps_zero] = 1
            field_plot[invalid_but_csa] = -999

        p = ax.pcolormesh(lon_bounds, lat_bounds, coherency,
        ...
        ax.text(0.05, 0.05, f'{percent_neg:3.0f}%', color=negative_colour, transform=ax.transAxes, horizontalalignment='center', fontsize=11)
        ax.text(0.13, 0.05, f'{percent_unsure:3.0f}%', color='#ba8e25', transform=ax.transAxes, horizontalalignment='center', fontsize=11)
        ax.text(0.21, 0.05, f'{percent_pos:3.0f}%', color=positive_colour, transform=ax.transAxes, horizontalalignment='center', fontsize=11)
...
def main():
    ...
    global_plots_with95ci(output_dirs, datasets, bands, seasons,
                          plot_raw_lags=True, plot_type=plot_type)

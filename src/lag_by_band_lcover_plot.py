...
def make_plots(output_dirs, datasets, bands, seasons, classes, plot_type="png"):
    ...
    filename = os.path.join(figures_dir, f"lag_by_band_lcover_plot.{plot_type}")
...
def main():
    ...
    bands = [tuple(b) for b in
             ul.get_this_or_that("bands", metadata["lags"], metadata["filter"])
    ]

    seasons = ul.get_this_or_that("seasons", metadata["lags"], metadata["spectra"])
    ...
    ###########################################################################
    # Load a map of land cover classes.
    ###########################################################################
    land_cover = load_land_cover(mapping=standard_mapping,
